...
#define KCol 7
#define KBtnW (kSCREEN_WIDTH * 0.8)/7
#define KBtnH (kSCREEN_WIDTH * 0.8)/7
#define KMaxCount 37
#define KBtnTag 100
#define KTipsW (kSCREEN_WIDTH * 0.2)
...
- (void)creatControl
{
    ...
    CGFloat yearBtnW = (kSCREEN_WIDTH * 0.8) * 0.3;
    CGFloat monthbtnW = (kSCREEN_WIDTH * 0.8) * 0.2;
    CGFloat todayBtnW = (kSCREEN_WIDTH * 0.8) * 0.3;
    CGFloat padding = (kSCREEN_WIDTH - KTipsW - yearBtnW - monthbtnW - todayBtnW - KBtnW * 2) * 0.25;
    
    //年份按钮
    HWOptionButton *yearBtn = [[HWOptionButton alloc] initWithFrame:CGRectMake((kSCREEN_WIDTH * 0.2), 0, yearBtnW, KBtnH)];
    ...
    //上一月
    UIButton *preBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(yearBtn.frame) + 5, (KBtnH - ((kSCREEN_WIDTH * 0.8) * 0.1))/2, (kSCREEN_WIDTH * 0.8) * 0.1, (kSCREEN_WIDTH * 0.8) * 0.1)];
    ...
    //下一月
    UIButton *nextBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(monthBtn.frame), (KBtnH - ((kSCREEN_WIDTH * 0.8) * 0.1))/2, (kSCREEN_WIDTH * 0.8) * 0.1, (kSCREEN_WIDTH * 0.8) * 0.1)];
...
- (void)reloadData
{
    ...
    for (int i = 0; i < KMaxCount; i++) {
        ...
        if (i < firstDay - 1 || i > totalDays + firstDay - 2) {
            ...
            btn.enabled = YES;
            [btn setTitle:[NSString stringWithFormat:@"%zd", i - (firstDay - 1) + 1] forState:UIControlStateNormal];
        }
    }
}
...
- (void)sureBtnOnClick
{
    ...
    if (_showTimePicker) {//%02ld:%02ld 时分 (long)_hour, (long)_minute
        date = [NSString stringWithFormat:@"%ld年%02ld月%02ld日", (long)_year, (long)_month, (long)_day];
    }else {
        date = [NSString stringWithFormat:@"%ld年%02ld月%02ld日", (long)_year, (long)_month, (long)_day];
    }
...
- (void)show
{
    [UIView animateWithDuration:0.3f animations:^{
        self.frame = CGRectMake(0,(kSCREEN_HEIGHT - self.bounds.size.height)/2 , kSCREEN_WIDTH , self.bounds.size.height);
    }];
}
...
{
    [UIView animateWithDuration:0.3f animations:^{
        self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, kSCREEN_WIDTH , self.bounds.size.height);
    }];
}
