...
//            }
//        
//            self.currentChoose = self.dataArr[self.dataArr.count/2];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (!pickerLabel){
        pickerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_WIDTH -20 , 50)];
...
- (NSMutableArray *)dataArr{
    if (!_dataArr) {
        ...
        [_dataArr addObject:@"中国银行"];
        [_dataArr addObject:@"中国银行1"];
        [_dataArr addObject:@"中国银行2"];
        [_dataArr addObject:@"中国银行3"];
    }
    return _dataArr;
}
@end
