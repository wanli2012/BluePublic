...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    CGFloat offset = 0;
    if (kbHeight < (self.height - CGRectGetMaxY(self.passwordF.frame))) {
        offset = 0;
    }else{
        offset = kbHeight - (kSCREEN_HEIGHT*0.5 - CGRectGetMaxY(self.passwordF.frame)) + 20;
    }
    ...
    if(offset > 0) {
        [UIView animateWithDuration:duration animations:^{
            self.frame = CGRectMake(0, kSCREEN_HEIGHT *0.5 - offset, self.frame.size.width, kSCREEN_HEIGHT *0.5);
...
- (void)keyboardWillHide:(NSNotification *)notify {
    ...
    //视图下沉恢复原状
    [UIView animateWithDuration:duration animations:^{
        self.frame = CGRectMake(0, kSCREEN_HEIGHT-self.frame.size.height, self.frame.size.width, self.frame.size.height);
    }];
}
